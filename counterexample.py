...
def compare_hh_optimal(m: int, n: int) -> Tuple[np.ndarray, float]:
    ...
    return shift_difference_hh1, revenue_hh1, shift_difference_hh2, revenue_hh2, rev_opt
...
def analyze_hh1_optimal(m: int, n: int, verbose=False):
    ...
    for _ in range(N):
        shift_diff_hh1, rev_hh1, shift_diff_hh2, rev_hh2, rev_opt = compare_hh_optimal(m, n)
    ...
    return prop_rev_deviation_hh1, prop_rev_deviation_hh2

if __name__ == '__main__':
    # size = np.arange(3, 13)
    # size = size.reshape(size.size, 1)
    # prop_dev = np.zeros((size.size, 2))
    # for i, s in enumerate(size):
    #     hh1, hh2 = analyze_hh1_optimal(int(s), int(s))
    #     prop_dev[i, 0] = hh1
    #     prop_dev[i, 1] = hh2

    # np.savetxt('hh1_hh2.csv', np.hstack((size, prop_dev)))
    data = np.loadtxt('hh1_hh2.csv')
    size = data[:, 0]
    prop_dev = data[:, [1, 2]]
    ...
    plt.plot(size, prop_dev[:, 1], label='HH2')
    plt.legend()
    plt.show()
